...
| 429 | Rate limit exceeded — `AuthRateLimit` (5 req/min) or `GlobalRateLimit` (200 req/min per user) |
...
- Global rate limit (B1 / R7): 200 req/min per IP sliding window on all `[Authorize]` controllers ✅
